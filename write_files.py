...
base = r'd:\cursor\cursor_programe1\zhilianweiji\frontend\src\pages'
...
const draw2D = () => {
  ...
  Object.values(pos).forEach(({ x, y, node }) => {
    ...
    ctx.fillText((node.title || '').substring(0, 8), x, y + 26)
  })
  ctx.restore()
}
...
const init3D = async () => {
  ...
  try { THREE = await import('three') } catch {
    is3D.value = false; init2D(); return
  }
  const canvas = canvasRef.value
  ...
  nodes.value.forEach((node, i) => {
    ...
    const color = parseInt(getNodeColor(node.type).replace('#', ''), 16)
...
.graph-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; }
...
with open(r'd:\\cursor\\cursor_programe1\\zhilianweiji\\frontend\\src\\pages\\KnowledgeGraph.vue', 'w', encoding='utf-8') as f:
    f.write(kg)

print('KnowledgeGraph.vue written')
